...
public class GiveCommand implements SubCommand {
    ...
    public GiveCommand(EconomyService economy, ConfigManager config, MessageConfig messages) {
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§c用法: /" + label + " give <玩家> <金额> [备注]");
            return;
        }
        ...
        EconomyResult result = economy.deposit(target.getUniqueId(), target.getName(), amount, source, remark);

        if (result.isSuccess()) {
            sender.sendMessage(messages.get("give-success")
            ...
            if (targetPlayer != null) {
                targetPlayer.sendMessage(messages.get("give-notify")
