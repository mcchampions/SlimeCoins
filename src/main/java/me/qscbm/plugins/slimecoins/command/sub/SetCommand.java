...
public class SetCommand implements SubCommand {
    ...
    public SetCommand(EconomyService economy, ConfigManager config, MessageConfig messages) {
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§c用法: /" + label + " set <玩家> <金额> [备注]");
            return;
        }
        ...
        EconomyResult result = economy.setBalance(target.getUniqueId(), target.getName(), amount, source, remark);

        if (result.isSuccess()) {
            sender.sendMessage(messages.get("set-success")
            ...
            if (targetPlayer != null) {
                targetPlayer.sendMessage(messages.get("set-notify")
