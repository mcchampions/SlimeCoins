...
public class MySQLProvider implements DataProvider {
    ...
    public MySQLProvider(String host, int port, String database, String username, String password, boolean useSSL) {
        this.host = host;
        this.port = port;
    ...
    public void init() throws Exception {
        ...
        String url = "jdbc:mysql://" + host + ":" + port + "/" + database + "?useSSL=" + useSSL + "&allowPublicKeyRetrieval=true";
    ...
    public void updateBalance(UUID uuid, String playerName, BigDecimal balance) {
        String sql = "UPDATE slimecoins_balance SET balance = ?, player_name = ? WHERE uuid = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    public void createAccount(UUID uuid, String playerName) {
        String sql = "INSERT INTO slimecoins_balance (uuid, player_name, balance, updated_at) VALUES (?, ?, 0.00, NOW())";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, uuid.toString());
            ps.setString(2, playerName);
            ps.executeUpdate();
    ...
    public void logTransaction(UUID playerUuid, String playerName, BigDecimal amount,
        ...
        String sql = "INSERT INTO slimecoins_log (player_uuid, player_name, amount, type, source, balance_after, remark, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
